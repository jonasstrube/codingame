...
while True:
    ...
    for i in range(zone_count):
        ...
        # print(str(zone_occupancy[i][0]) + " | " + str(zone_occupancy[i][1]) + " | " + str(zone_occupancy[i][2]) + " | " + str(zone_occupancy[i][3]) + " | " + str(zone_occupancy[i][4]) + " | " + str(zone_occupancy[i][5]), file=sys.stderr, flush=True)

    # ----------------------ACTIONS--------------------------
    moves_command = ""
    for i in range(len(zone_occupancy)):
        print("I calculate zone " +str(i) + "!", file=sys.stderr, flush=True)
        if zone_occupancy[i][1] == my_id:       # if zone is occupied by me
            print("Zone " + str(i) + " is occupied by me!", file=sys.stderr, flush=True)
            for j in range(len(links)):
                neighbour = None
                if links[j][0] == i:        # occupied zone is linked to other zone
                    neighbour = links[j][1]
                elif links[j][1] == i:      # occupied zone is linked to other zone
                    neighbour = links[j][0]
                if neighbour != None:
                    if zone_occupancy[neighbour][1] == -1: # neighbour zone is neutral
                        moves_command = moves_command + (str(1) + " " + str(i) + " " + str(links[j][0]) + " ") # move one pod from my zone to neutral neighbour zone
